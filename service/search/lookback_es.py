...
class LookbackLib:
    def __init__(self, args, conn, user_lib, user_id):
        global user_data
        #可能不用
        user_data = utils.common_sql.user_data
        #if utils.common_sql.if_async:
        #    print('USING AYNC USERLIB FOR LOOKBACK')
        #    # from utils.common_sql import AsyncLib as Userlib_
        #    self.userlib = utils.common_sql.AsyncLib(user_id)
        #else:
        #    print('USING SYNC USERLIB FOR LOOKBACK')
        #    # from utils.common_sql import UserLib as Userlib_
        #    userlib = utils.common_sql.UserLib()
        ...
        if agg_tag:
            ...
            if not res:
                res = await self.conn.search(index=self.indexes, body=_dsl,
                                             sort=['id:desc'], timeout='2s', from_=_offset, size=_limit)
    ...
    async def history_muc(self, user):
        ...
        if not res:
            ...
            for sli in user_mucs:
                ...
                if agg_tag:
                    ...
                    t = self.conn.search(index=self.indexes, df='muc_msg', body=_dsl,
                                         sort=['id:desc'], timeout='2s', from_=_offset,
                                         size=_limit)
                    tasks.append(t)
                else:
                    _limit = self.limit + 1
            ...
            _coro_time = time.time()
            coro_result = await asyncio.gather(*tasks)
            _coro_time = time.time() - _coro_time
    ...
    async def history_file(self, user):
        ...
        for sli in user_mucs:
            ...
            _limit = self.limit + 1
            _offset = self.offset
            t = self.conn.search(index=self.indexes, body=_dsl, sort='date:desc', timeout='2s',
