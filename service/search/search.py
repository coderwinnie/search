            ...
            if ckey:
                if auth_ckey_url:
                    try:
                        ...
                        if ret.json().get('ret') and ret.json().get('data',{}).get('u') == user_id:
...
@search_blueprint.route('/searchecho', methods=['GET', 'POST'])
def echo():
    return "OK"
...
def main(user_id, args):
    ...
    if (_group_id or _group_id == '') and 'action' not in args:
        ...
        if 'platform' in args and args.get('platform').lower() == 'ios':  # 此处等ios兼容后就删掉
            action = '7'
        else:
...
async def go_coro(if_contact, if_lookback, args, user, habit):
    ...
    if if_lookback and if_use_lookback:
        ...
        if if_lookback:
            ...
            tasks.append(t)
    completed, pending = await asyncio.wait(tasks, timeout=20)
    for pen in pending:
