        ...
        if key in ['chat', 'groupchat', 'consult']:
            try:
                ...
                if not _id:
                    ...
                    es_logger.error(msg)
                    with open(os.path.abspath(os.path.dirname(__file__)) + '/log/idfailed.log', 'a+') as f:
                        f.write('{}\n'.format(_msg_id))
                    continue
            ...
            try:
                ...
                if not _id:  # 如果查看es已有一条msgtype=-1的就不导入了
                    with open(os.path.abspath(os.path.dirname(__file__)) + '/log/idfailed.log', 'a+') as f:
                        f.write('{}\n'.format(_msg_id))
...
def handle_body(content):
    ...
    if extendinfo:
        ...
        if isinstance(extendinfo, dict):
            ...
            es_logger.warning('extendinfo type error: {}'.format(extendinfo))
