...
class UserLib:
    ...
    def get_msg_id(self, msgid, msgtype):
        ...
        sql = """SELECT id FROM {} where msg_id = %(msgid)s and from_host = %(domain)s limit 1;""".format(msg_table)
        cursor = conn.cursor()
        cursor.execute(sql, {'msgid': msgid, 'domain': self.domain})
        rs = cursor.fetchall()
        if len(rs):
            res = rs[0][0]
        else:
    ...
    def get_user_mucs(self, user_id, user_domain=''):
        s_result = []
        conn = self.conn
        sql = "select muc_name, domain from user_register_mucs where username =%(user_id)s and registed_flag = 1 and host = %(domain)s;"
        cursor = conn.cursor()
        cursor.execute(sql, {'user_id': user_id, 'domain': user_domain})
        rs = cursor.fetchall()
        for row in rs:
            row = ['' if x is None else x for x in row]
            res = {row[0], row[1]}
    ...
    def get_habit(self, key, habit, form, user):
        result = []
        # 搜人的拼音和userid
        if '@' not in user:
            return
        __domain = user.split('@')[1]
        if form == 'single':
            ...
            if __user_data:
                ...
                __user_data = self.single_habit_data(data=user_list, user_domain=__domain)
                try:
            ...
            if __user_data:
                ...
                if not chinese_pattern.sub('', key):
                    # result = set(filter(lambda x: key in x['name'], __user_data))
                    result = merge_list_of_dict(list((filter(lambda x: key in x['name'], __user_data))))
                # 搜索userid
                    ...
                    _r2 = list(filter(lambda x: key in x['qtalkname'], __user_data))
                    _r3 = list(filter(lambda x: key in x['pinyin'], __user_data))
        ...
        # 搜群的id 和拼音 和 title
            ...
            if __muc_data:
                ...
                try:
                    __muc_data = sorted(__muc_data, key=lambda x: muc_list.index(x.get('uri').split('@')[0]))
                except ValueError:
                    sql_logger.exception("ORDER PROBLEM : NOT IN LIST")
            ...
            if __muc_data:
                ...
                if not chinese_pattern.sub('', key):
                    ...
                    result = merge_list_of_dict(list((filter(lambda x: key in x['label'], __muc_data))))
                # 搜索userid
                # elif ('.' in key) or ('_' in key):
                #     result = set(filter(lambda x: key in x['uri'], __muc_data))
                else:
                    _r1 = list(filter(lambda x: key in x['label'].lower(), __muc_data))
                    ...
                    _r3 = list(filter(lambda x: reduce(lambda a, b: a + b, list(
                        map(lambda x: True if key in x else False, pinyin.get_all(x['label'])))), __muc_data))
                    result = merge_list_of_dict(_r1, _r2, _r3)
        # self.close()
    ...
    def muc_habit_data(self, data, user, user_domain=''):
        ...
        s_data = data
        sql = """select a.muc_name, a.domain, b.show_name, b.muc_title, b.muc_pic from user_register_mucs as a left join muc_vcard_info as b on concat(a.muc_name, '@', a.domain) = b.muc_name where a.registed_flag != 0 and a.username = %(user_id)s and (b.muc_name in %(muc_list)s) and a.host = %(domain)s"""
        cursor = conn.cursor()
        cursor.execute()

        cursor.execute(sql, {'muc_list': s_data, 'user_id': user, 'domain': user_domain})
        rs = cursor.fetchall()
        for row in rs:
            row = ['' if x is None else x for x in row]
            res = dict()
    ...
    def search_group(self, user_id, username, limit=5, offset=0, habit='', exclude=None, origin=True,
        ...
        cursor.close()
        __muc_end_time = time.time()
        sql_logger.info('MUC SQL USE TIME {}'.format(__muc_end_time - __muc_start_time))
        if not from_habit:
            if if_cached and habit:
    ...
    (m_to = %(user_s_name)s and to_host = %(user_domain)s and m_from = %(to_user_s_name)s and from_host = %(to_user_domain)s ) 
            ...
            sql = sql.format(user_limit=user_limit, time_limit_start=time_limit_start, time_limit_end=time_limit_end,search_model=search_model)
            cursor = conn.cursor()
            cursor.execute(sql,
        ...
        if not agg_tag and to_muc:
            ...
            if time_range and isinstance(time_range, list):
                ...
                if time_range[1]:
                    time_limit_end = "AND create_time < %(time_limit_starts)s"
                    sub_injection['time_limit_end'] = time_range[1]
            sql = sql.format(muc_limit=muc_limit, time_limit_start=time_limit_start, time_limit_end=time_limit_end,search_model=search_model)
            cursor = conn.cursor()
            cursor.execute(sql,
                           {**{'term': term, 'limit': limit, 'offset': offset, 'user_id': user_id},
                            **sub_injection})
...
class AsyncLib:
    ...
    async def get_user_mucs(self, user_id):
        ...
        s_result = list()
        sql = "SELECT muc_name||'@'||domain from user_register_mucs where username = $1 and registed_flag = 1 and domain = $2"
        pgconn = await asyncpg.connect(self.conn_str)
        injection = [user_id, conference_str]
        stmt = await pgconn.prepare(sql)
    ...
    async def get_habit(self, key, habit, form, user, origin=False, common=False):
        if '@' in user:
            ...
            return []
        if not self.user_data:
            self.user_data = await self.get_user_data(user.split('@')[1])
            if self.user_data:
                cache_redis_cli = RedisUtil()
                cache_redis_cli.set_all_user_data(data=self.user_data, domain=__domain)
                sql_logger.info("redis user data set..")
        ...
        if form == 'single':
            ...
            if __user_data:
                ...
                __user_data = await self.single_habit_data(user_list)
                try:
                    __user_data = sorted(__user_data, key=lambda x: user_list.index(x.get('qtalkname', '')))
                except ValueError:
                    sql_logger.exception("ORDER PROBLEM : NOT IN LIST")
            ...
            if __user_data:
                ...
                if not chinese_pattern.sub('', key):
                ...
                elif chinese_pattern.sub('', formulate_text(key)):  # 中英符号结合
                    key = formulate_text(key)
                    _r1 = list(filter(lambda x: key in formulate_text(x['name']), __user_data))  # 何靖宇
                    _r2 = list(filter(lambda x: key in x['qtalkname'], __user_data))  # jingyu.he
                    ...
                    _r4 = list(filter(lambda x: key in formulate_text(x['pinyin']), __user_data))
                    sql_logger.debug('r4 {}'.format(_r4))
        ...
        # 搜群的id 和拼音 和 title
            ...
            if __muc_data:
                ...
                #     result = set(filter(lambda x: key in x['uri'], __muc_data))
                    ...
                    key = formulate_text(key)
                    _r1 = list(filter(lambda x: key in formulate_text(x['label']), __muc_data))
                    sql_logger.debug('R1 {}'.format(_r1))
                    # 群名称的拼音 后续撤掉
                    # 先取每个结果的label 得到[拼音,首字母]的结果 之后用map分别得到key是否在里 再用reduce进行或操作
                    _r2 = list(filter(lambda x: reduce(lambda a, b: a + b, list(
                        map(lambda x: True if key in x else False, pinyin.get_all(formulate_text(x['label']))))),
                                      __muc_data))
                    chinese_words = chinese_pattern.findall(key)
                    __k = list(map(lambda x: pinyin.get_pinyin(x), chinese_words))
                    test = {f: t for f, t in zip(chinese_words, __k)}.items()
                    ...
                    _r3 = list(filter(lambda x: reduce(lambda a, b: a + b, list(
                        map(lambda x: True if key in x else False, pinyin.get_all(formulate_text(x['label']))))),
                                      __muc_data))
                    if common:
                        _r4 = await self.search_group(user_id=user, username=raw_key, limit=len(__muc_list), offset=0,
                                                      habit='', exclude=__muc_list, origin=origin, common=common,
                                                      from_habit=True)
                        result = merge_list_of_dict(_r1, _r2, _r3, _r4)
                    else:
                    ...
                    if common:
                        ...
                        result = merge_list_of_dict(_r1, _r2, _r3)
                    sql_logger.debug(
                        'PINYIN {}'.format([formulate_text(pinyin.get_all(x['label'])) for x in __muc_data]))
                    sql_logger.debug('R3 {}'.format(_r3))
        # self.close()
        sql_logger.debug('returning result {}'.format(list(result)))
        return list(result)

    async def single_habit_data(self, data):
        s_result = list()
        sql = """SELECT aa.user_id, aa.department, aa.icon, aa.user_name, aa.mood, aa.pinyin FROM ( SELECT a.user_id, a.department, b.url AS icon, a.user_name, b.mood, a.pinyin FROM host_users a LEFT JOIN vcard_version b ON a.user_id = b.username WHERE a.hire_flag = 1 AND LOWER(a.user_type) != 's' AND a.user_id = ANY($1)) aa """

        pgconn = await asyncpg.connect(self.conn_str)
        stmt = await pgconn.prepare(sql)
        for (user_id, department, icon, user_name, mood, __pinyin) in await stmt.fetch(data):
            res = dict()
    ...
    async def muc_habit_data(self, data, user):
        s_result = list()
        #        s_data = list(map(lambda x: x + '@conference.' + domain, data))
        s_data = data
        sql_logger.debug('WATCH ORDER {}'.format(data))
        # s_data = ','.join(list(map(lambda x: x + '@conference.' + domain, data)))
        sql = """select a.muc_name, a.domain, b.show_name, b.muc_title, b.muc_pic, b.show_name_pinyin from user_register_mucs as a left join muc_vcard_info as b on concat(a.muc_name, '@', a.domain) = b.muc_name where a.registed_flag != 0 and a.username = $2 and a.muc_name = ANY($1)"""
        injection = [s_data, user]
        # 下面这个注解可取回群成员
    ...
    async def get_all_users(self):
        s_result = list()
        sql = "SELECT user_id FROM host_users WHERE hire_flag = 1;"
        pgconn = await asyncpg.connect(self.conn_str)
        stmt = await pgconn.prepare(sql)
        for (user_id) in await stmt.fetch():
            s_result.append(user_id)
        await pgconn.close()
        return s_result
    ...
    async def search_group(self, user_id, username, limit=5, offset=0, habit='', exclude=None, origin=True,
        ...
        if from_habit:
            ...
            injection = [user_id, user_s_name, limit, offset, exclude_list, user_domain]
        pgconn = await asyncpg.connect(self.conn_str)
        stmt = await pgconn.prepare(sql)
        __muc_start_time = time.time()
    ...
    """
        ...
        pgconn = await asyncpg.connect(self.conn_str)
        stmt = await pgconn.prepare(sql)
        for (_file, from_, to_, date, msgid, label, icon, msg) in await stmt.fetch(*injection):
            row = [_file, from_, to_, date, msgid, label, icon, msg]
            row = ['' if x is None else x for x in row]
            res = dict()
        ...
        if origin and common:
            ...
            sql = """SELECT
                     ...
                     ON 
                        concat(a.muc_name, '@', a.domain) = b.muc_name
                     WHERE 
                        a.registed_flag != 0 and a.username = $1 and (b.show_name {search_model} $2 or b.muc_name ~ $2) and b.muc_name <> ALL ($5) AND a.host = ${searcher_domain_index}
                     order by b.update_time desc limit $3 offset $4"""
        return sql
...
if DB_VERSION is None:
    ...
    # conf.constants.DB_VERSION = DB_VERSION
    # constants.DB_VERSION = DB_VERSION
