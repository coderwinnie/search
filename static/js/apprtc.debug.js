...
var UI_CONSTANTS = {confirmLeaveButton:"#confirm-leave-button", confirmJoinButton:"#confirm-join-button", confirmJoinDiv:"#confirm-join-div", confirmJoinRoomSpan:"#confirm-join-room-span", fullscreenSvg:"#fullscreen", hangupSvg:"#hangup", icons:"#icons", infoDiv:"#info-div", localVideo:"#local-video", cutVideo:"#cut-video", miniVideo:"#mini-video", cutScreenSvg:"#cut-screen", drawCanvasSvg:"#draw-canvas", muteAudioSvg:"#mute-audio", muteVideoSvg:"#mute-video", newRoomButton:"#new-room-button", newRoomLink:"#new-room-link", 
privacyLinks:"#privacy", remoteVideo:"#remote-video", rejoinButton:"#rejoin-button", rejoinDiv:"#rejoin-div", rejoinLink:"#rejoin-link", roomLinkHref:"#room-link-href", roomSelectionDiv:"#room-selection", roomSelectionInput:"#room-id-input", roomSelectionInputLabel:"#room-id-input-label", roomSelectionJoinButton:"#join-button", roomSelectionRandomButton:"#random-button", roomSelectionRecentList:"#recent-rooms-list", sharingDiv:"#sharing-div", statusDiv:"#status-div", videosDiv:"#videos", mobileInputDiv:"#mobileInput", 
mobileInputBtn:"#sendInputBtn", mobileInputContent:"#sendInput"};
var AppController = function(loadingParams) {
  trace("all params" + loadingParams);
  trace("Initializing; server= " + loadingParams.roomServer + ".");
  trace("Initializing; room=" + loadingParams.roomId + ".");
  trace("caller:" + loadingParams.caller);
  trace("callee:" + loadingParams.callee);
  trace("u:" + loadingParams.user);
  trace("calltime:" + loadingParams.callTime);
  this.hangupSvg_ = $(UI_CONSTANTS.hangupSvg);
  this.cutScreenSvg_ = $(UI_CONSTANTS.cutScreenSvg);
  this.drawCanvasSvg_ = $(UI_CONSTANTS.drawCanvasSvg);
  this.icons_ = $(UI_CONSTANTS.icons);
  this.localVideo_ = $(UI_CONSTANTS.localVideo);
  this.cutVideo_ = $(UI_CONSTANTS.cutVideo);
  ...
  this.mobileInput_ = $(UI_CONSTANTS.mobileInputDiv);
  ...
  this.cutScreenIconSet_ = new AppController.IconSet_(UI_CONSTANTS.cutScreenSvg);
  window.addEventListener("resize", this.onVideoSizeChanged.bind(this), false);
  this.isRemoteMobile = false;
  this.isPc = loadingParams.isPc;
  this.hide_(this.drawCanvasSvg_);
  if (!this.isPc) {
  }
  ...
  this.tempArray = [];
  this.resolutionDict = {};
  this.resolutionDict["localw"] = window.outerWidth;
  this.resolutionDict["localh"] = window.outerHeight;
  window.resolutionDict = this.resolutionDict;
  this.miniVideoClone = null;
  ...
  Promise.resolve(paramsPromise).then(function(newParams) {
    ...
    this.sharingDivResetTimer_ = null;
    this.loadingParams_.entrance = null;
    if (this.loadingParams_.caller && this.loadingParams_.callee && this.loadingParams_.user) {
      this.loadingParams_.entrance = "qtalk";
      this.loadingParams_.user = this.loadingParams_.user.replace(/\./g, "");
      this.loadingParams_.caller = this.loadingParams_.caller.replace(/\./g, "");
      this.loadingParams_.callee = this.loadingParams_.callee.replace(/\./g, "");
      this.loadingParams_.clientId_ = this.loadingParams_.user;
      this.loadingParams_.clientId = this.loadingParams_.user;
      console.log(this.loadingParams_);
      var callers = [this.loadingParams_.caller.replace(/\./g, ""), this.loadingParams_.callee.replace(/\./g, "")];
      var sortCallers = callers.sort(function(a, b) {
        return a.toString().localeCompare(b);
      });
      this.loadingParams_.roomId = sortCallers.join("_");
    } else {
      this.loadingParams_.entrance = "entrance";
    }
    if (this.loadingParams_.roomId) {
      this.createCall_();
      if (this.loadingParams_.entrance === "qtalk" && this.loadingParams_.callee === this.loadingParams_.user) {
        var channalPromise = this.call_.channel_.open();
        Promise.resolve(channalPromise).then(function() {
          this.call_.channel_.wait(this.loadingParams_.roomId, this.loadingParams_.user);
        }.bind(this));
      ...
      $(UI_CONSTANTS.confirmLeaveButton).onclick = function() {
        this.hide_(confirmJoinDiv);
        if (!this.call_.pcClient_ && !this.call_.channel_.registered_ && this.call_.channel_.websocket_.readyState < 1) {
          this.call_.channel_.open();
        }
        console.log("roomId is " + this.loadingParams_.roomId);
        this.hangup_();
        history.replaceState({}, {}, goHangupUrl());
      }.bind(this);
      if (this.loadingParams_.callee === this.loadingParams_.user) {
        console.log("im callee");
        this.show_(confirmJoinDiv);
      } else {
        if (this.loadingParams_.caller === this.loadingParams_.user) {
          this.sharingDiv_ = $(UI_CONSTANTS.sharingDiv);
...
AppController.prototype.createCall_ = function() {
  ...
  this.call_.ontrackremoved = this.onTrackRemoved_.bind(this);
  ...
  this.call_.onDataChannel_ = this.onDataChannel_.bind(this);
  this.call_.dataChannel = this.dataChannel;
  this.call_.syncCanvasData_ = this.syncCanvasData_.bind(this);
};
...
AppController.prototype.toggleOpacity = function() {
  if (this.sharingDivResetTimer_) {
    window.clearTimeout(this.sharingDivResetTimer_);
  }
  this.sharingDiv_.style.opacity = .8;
  this.sharingDivResetTimer_ = window.setTimeout(function() {
    this.sharingDiv_.style.opacity = .2;
    this.sharingDivResetTimer_ = null;
  }.bind(this), 5000);
};
AppController.prototype.setupUi_ = function() {
  ...
  if (this.isPc) {
    window.addEventListener("keyup", this.onKeyPress_.bind(this));
    ...
    window.ontouchstart = function(e) {
      if (Number(e["touches"][0]["pageY"]) < Number(this.sharingDiv_.offsetTop)) {
        this.showIcons_();
      } else {
        this.toggleOpacity();
      }
    }.bind(this);
    this.sharingDiv_.style.maxHeight = "20%";
    this.sharingDiv_.style.opacity = .2;
    this.toggleOpacity();
  }
  ...
  $(UI_CONSTANTS.miniVideo).onclick = this.changeMiniVideo_.bind(this);
  $(UI_CONSTANTS.cutVideo).onclick = this.changeCutVideo_.bind(this);
  $(UI_CONSTANTS.cutScreenSvg).onclick = this.toggleCutScreen.bind(this);
  $(UI_CONSTANTS.drawCanvasSvg).onclick = this.toggleCanvasScreen.bind(this);
  setUpFullScreen();
};
...
AppController.prototype.hangup_ = function() {
  ...
  if (loadingParams.entrance === "qtalk") {
    history.replaceState({}, {}, goHangupUrl());
  }
};
...
AppController.prototype.onRemoteSdpSet_ = function(hasRemoteVideo) {
  ...
  if (!this.isPc) {
    var sendData_;
    sendData_ = {type:"isMobile", data:true};
    this.call_.channel_.send(JSON.stringify(sendData_));
  }
  if (this.sharingDiv_) {
    if (document.getElementById("room-link")) {
      this.sharingDiv_.removeChild(document.getElementById("room-link"));
      if (this.copyLinkBtn) {
        this.sharingDiv_.removeChild(this.copyLinkBtn);
      }
    }
  }
  if (this.infoBox_.localTrackIds_.video === "" && this.infoBox_.remoteTrackIds_.video === "") {
    this.hide_(this.miniVideo_);
    this.hide_(this.cutVideo_);
  }
};
AppController.prototype.onTrackRemoved_ = function() {
  if (this.cutVideo_.classList.contains("active")) {
    this.cutVideo_.classList.remove("active");
  }
  this.remoteVideo_.srcObject = this.call_.pcClient_.pc_.getRemoteStreams()[0];
  this.miniVideo_.srcObject = this.infoBox_.localVideoSrc;
};
...
AppController.prototype.onRemoteStreamAdded_ = function(streams) {
  if (typeof streams == "object" && streams.length === 1) {
    if (this.isPc) {
      this.deactivate_(this.sharingDiv_);
    }
    trace("Remote stream added.");
    var stream = streams[0];
    var pc = this.call_.pcClient_.pc_;
    var newStream = false;
    if (pc.getRemoteStreams().length === 1) {
      newStream = true;
      trace("new stream!");
    } else {
      if (pc.getRemoteStreams().length === 2) {
        if (this.call_.pcClient_.params_.remoteScreenShare) {
          this.cutVideo_.classList.add("active");
        }
        pc.getRemoteStreams().forEach(function(st) {
          if (this.call_.pcClient_.params_.remoteScreenShare) {
            if (st.getAudioTracks().length === 0) {
              this.cutVideo_.classList.add("active");
              this.remoteVideo_.srcObject = st;
              this.infoBox_.remoteScreenShareSrc = st;
            } else {
              this.cutVideo_.srcObject = st;
              this.cutVideo_.muted = false;
            }
            this.miniVideo_.srcObject = this.infoBox_.localVideoSrc;
          }
        }.bind(this));
    ...
    this.infoBox_.remoteVideoSrc = stream;
  } else {
    if (typeof streams == "object" && streams.length === 2) {
      trace("Remote streams detected, Checking streams...");
      if (this.isPc) {
        this.deactivate_(this.sharingDiv_);
      }
      var nowStreams_;
      nowStreams_ = this.call_.pcClient_.pc_.getRemoteStreams();
      if (nowStreams_.length === 1) {
        streams.forEach(function(st) {
          if (st.id === nowStreams_[0].id) {
            trace("video stream got");
            this.cutVideo_ = st;
            this.infoBox_.getRemoteTrackIds(stream);
            this.infoBox_.remoteVideoSrc = stream;
          } else {
            trace("screenshare stream got");
            this.miniVideo_.srcObject = st;
            this.remoteVideo_ = st;
            this.infoBox_.remoteScreenShareSrc = st;
          }
        }.bind(this));
      }
  ...
  var sendData_;
  sendData_ = {type:"resolution", data:this.resolutionDict};
  this.call_.channel_.send(JSON.stringify(sendData_));
};
...
AppController.prototype.transitionToActive_ = function() {
  ...
  if (!this.miniVideo_.srcObject) {
    this.miniVideo_.srcObject = this.localVideo_.srcObject;
  } else {
    trace("mini video has source");
  }
  ...
  if (this.hangupSvg_.classList.contains("hidden")) {
    this.show_(this.hangupSvg_);
  }
  if (this.loadingParams_.isInitiator === true) {
    this.show_(this.cutScreenSvg_);
    if (this.isPc) {
      this.show_(this.drawCanvasSvg_);
    }
  }
  if (!this.isPc) {
    this.activate_(this.mobileInput_);
    $(UI_CONSTANTS.mobileInputBtn).onclick = function() {
      var data_ = $(UI_CONSTANTS.mobileInputContent).value;
      if (data_ === "") {
        this.showChatMsg_();
        return;
      }
      this.syncChatMsg(data_);
      var sendData_;
      sendData_ = {type:"chat", data:data_};
      this.call_.channel_.send(JSON.stringify(sendData_));
      $(UI_CONSTANTS.mobileInputContent).value = "";
      this.toggleOpacity();
      this.moveIcons_();
    }.bind(this);
  }
...
AppController.prototype.onCanvasKeyPress_ = function(key_) {
  if (!this.call_.channel_.registered_) {
    return;
  }
  var data_;
  if (key_ === 13) {
...
AppController.prototype.onKeyPress_ = function(event) {
  if (!this.call_.channel_.registered_ || !this.isPc) {
    return;
  }
  var data_;
  var key_ = event.which || event.keyCode;
  if (!this.isPc) {
    return;
  }
  if (key_ === 13) {
    ...
    if (key_ === 32) {
      if (this.sharingDiv_.classList.contains("active")) {
        this.sharingDivResetTimer_ = null;
        this.deactivate_(this.sharingDiv_);
      } else {
        this.showChatMsg_();
...
AppController.prototype.displaySharingInfo_ = function(roomId, roomLink) {
  roomLink = roomLink.replace(/apprtc\.domain\.com\//, "apprtc.domain.com:8083/");
  this.clipb = new ClipboardJS("#copyLinkBtn");
  this.clipb.on("success", function() {
    this.copyLinkBtn.style.backgroundColor = "#383d46";
  }.bind(this));
  this.copyLinkBtn = document.createElement("button");
  this.copyLinkBtn.innerHTML = "\u590d\u5236\u623f\u95f4\u94fe\u63a5";
  this.copyLinkBtn.id = "copyLinkBtn";
  this.copyLinkBtn.setAttribute("data-clipboard-action", "copy");
  this.copyLinkBtn.setAttribute("data-clipboard-target", "#room-link-href");
  this.copyLinkBtn.setAttribute("data-clipboard-text", roomLink);
  this.sharingDiv_.appendChild(this.copyLinkBtn);
...
AppController.prototype.toggleVideoMute_ = function() {
  if (this.loadingParams_.mediaConstraints.video === false) {
    this.call_.params_.mediaConstraints.video = true;
    if (this.call_.pcClient_.started_ && this.infoBox_) {
      this.call_.maybeGetMedia_().then(function() {
        this.localVideo_.srcObject = this.infoBox_.localVideoSrc;
        this.call_.sendSignalingMessage_({"type":"mediaAdd", "mediaType":"c"});
        this.miniVideo_.srcObject = this.infoBox_.localVideoSrc;
        this.call_.pcClient_.pc_.addTrack(this.infoBox_.localVideoSrc.getVideoTracks()[0], this.call_.pcClient_.pc_.getLocalStreams()[0]);
      }.bind(this));
    }
  } else {
    this.call_.toggleVideoMute();
...
AppController.prototype.toggleCanvasScreen = function() {
  if (!this.call_) {
    return;
  }
  if (!this.designer) {
    ...
    this.designer = null;
    if (this.remoteVideo_.classList.contains("hidden")) {
      this.localVideo_.classList.remove("hidden");
    }
    if (this.cutVideo_.classList.contains("hidden")) {
      this.cutVideo_.classList.remove("hidden");
    }
    this.activate_(document.getElementById("remote-video"));
    this.activate_(document.getElementById("local-video"));
    this.activate_(this.miniVideo_);
    document.body.removeChild(document.getElementById("mini-video-clone"));
    this.call_.channel_.send("destroyCanvas");
  }
};
AppController.prototype.toggleCutScreen = function() {
  if (this.call_.isCutScreen && this.loadingParams_.mediaConstraints.video) {
    if (this.cutVideo_.classList.contains("active")) {
      this.cutVideo_.classList.remove("active");
      if (this.call_.pcClient_ && this.infoBox_.remoteVideoSrc && this.infoBox_.localVideoSrc && this.infoBox_.localScreenShareSrc) {
        this.remoteVideo_.srcObject = this.infoBox_.remoteVideoSrc;
        this.miniVideo_.srcObject = this.infoBox_.localVideoSrc;
        var screenShareTrack = this.call_.getScreenShareTrack(this.infoBox_.localScreenShareSrc);
        this.call_.removeTrack(screenShareTrack);
        this.infoBox_.localScreenShareSrc = null;
        this.remoteVideo_.srcObject = this.infoBox_.remoteVideoSrc;
        this.miniVideo_.srcObject = this.infoBox_.localVideoSrc;
      } else {
        if (!this.call_.pcClient_ && this.infoBox_.localVideoSrc) {
          this.localVideo_.srcObject = this.localStream_.srcObject;
        } else {
          console.log("un expected situation");
          console.log("localStream is " + !!this.localStream_.srcObject);
          console.log("pcclient is " + !!this.call_.pcClient_);
        }
      }
    } else {
      console.log("un expected situation");
    ...
    this.miniVideo_.muted = true;
    this.call_.isCutScreen = false;
  } else {
    if (!this.cutVideo_.classList.contains("active") && this.infoBox_) {
      this.cutVideo_.classList.add("active");
      navigator.mediaDevices.getDisplayMedia().then(function(stream) {
        this.remoteVideo_.srcObject = stream;
        this.infoBox_.localScreenShareSrc = stream;
        this.call_.isCutScreen = true;
        this.call_.sendSignalingMessage_({"type":"mediaAdd", "mediaType":"s"});
        this.cutVideo_.srcObject = this.infoBox_.localVideoSrc;
        if (this.infoBox_.remoteVideoSrc) {
          this.miniVideo_.srcObject = this.infoBox_.remoteVideoSrc;
        }
      }.bind(this)).then(function() {
        this.call_.pcClient_.pc_.addTrack(this.infoBox_.localScreenShareSrc.getVideoTracks()[0], this.call_.pcClient_.pc_.getRemoteStreams()[0]);
      }.bind(this));
      this.call_.onCutScreenSuccess_();
    } else {
      if (this.cutVideo_.classList.contains("active")) {
        console.log("un expected situation");
        console.log("localStream is " + !!this.localStream_.srcObject);
  ...
  this.miniVideo_.muted = false;
  this.cutScreenIconSet_.toggle();
};
...
AppController.prototype.changeMiniVideo_ = function() {
  if (this.miniVideo_ && this.miniVideo_.classList.contains("active")) {
    var tmpSrcObj;
    tmpSrcObj = this.infoBox_.localVideo_.srcObject;
    this.infoBox_.localVideo_.srcObject = this.infoBox_.remoteVideo_.srcObject;
    this.infoBox_.remoteVideo_.srcObject = tmpSrcObj;
  } else {
    if (this.miniVideoClone && this.miniVideoClone.classList.contains("active")) {
      if (this.miniVideoClone.srcObject === this.infoBox_.localVideo_.srcObject) {
        this.miniVideoClone.srcObject = this.infoBox_.remoteVideo_.srcObject;
      } else {
        this.miniVideoClone.srcObject = this.infoBox_.localVideo_.srcObject;
      }
    }
  }
};
AppController.prototype.changeCutVideo_ = function() {
  if (this.cutVideo_.classList.contains("active")) {
    var tmpSrcObj;
    tmpSrcObj = this.remoteVideo_.srcObject;
    this.remoteVideo_.srcObject = this.cutVideo_.srcObject;
    this.cutVideo_.srcObject = tmpSrcObj;
  }
};
...
AppController.prototype.moveIcons_ = function() {
  this.icons_.style.bottom = this.sharingDiv_.offsetHeight + "px";
  if (this.miniVideo_) {
    this.miniVideo_.style.bottom = this.sharingDiv_.offsetHeight + "px";
  }
  if (this.miniVideoClone) {
    this.miniVideoClone.style.bottom = this.sharingDiv_.offsetHeight + "px";
  }
  if (this.cutVideo_) {
    this.cutVideo_.style.bottom = this.sharingDiv_.offsetHeight + "px";
  }
};
AppController.prototype.showChatMsg_ = function() {
  if (document.getElementById("room-link")) {
    this.sharingDiv_.removeChild(document.getElementById("room-link"));
    if (this.copyLinkBtn) {
      this.sharingDiv_.removeChild(this.copyLinkBtn);
  ...
  if (this.hideChatMsgAfterTimeout) {
    window.clearTimeout(this.hideChatMsgAfterTimeout);
  }
  this.activate_(this.sharingDiv_);
  this.hideChatMsgAfterTimeout = window.setTimeout(function() {
    this.hideChatMsg_();
  }.bind(this), 5000);
};
AppController.prototype.hideChatMsg_ = function() {
  if (this.sharingDiv_.classList.contains("active")) {
    this.deactivate_(this.sharingDiv_);
  }
};
AppController.prototype.setChatTimeout_ = function() {
  if (this.hideChatMsgAfterTimeout) {
    window.clearTimeout.bind(this, this.hideChatMsgAfterTimeout);
  }
  this.activate_(this.sharingDiv_);
  this.hideChatMsgAfterTimeout = window.setTimeout(function() {
    this.hideChatMsg_();
...
AppController.prototype.loadUrlParams_ = function() {
  ...
  this.loadingParams_.isCutScreen = urlParams["screenshare"] === "true";
  this.loadingParams_.isKurento = urlParams["k"] === "true";
};
AppController.prototype.syncCanvasData_ = function(data) {
  if (this.call_.channel_.websocket_.readyState !== 1) {
    return;
  }
  if (!this.designer) {
    this.newDesigner();
    this.onFinishDesigner();
  }
  this.designer.syncData(data);
};
AppController.prototype.onFinishDesigner = function() {
  if (!this.designer) {
    console.error("not finished designer");
    return;
  }
  if (this.call_.channel_.websocket_.readyState !== 1) {
    console.error("not finished websocket");
    return;
  }
  this.hide_(document.getElementById("remote-video"));
  this.hide_(document.getElementById("local-video"));
  this.designer.appendTo(document.getElementById("videos"));
  this.activate_(document.getElementById("canvasDesigner"));
  if (!this.isPc) {
    return;
  }
  this.designer.addSyncListener(function(data) {
    console.log(data);
    var sendData_;
    sendData_ = {type:"canvas", data:data};
    try {
      window.appController.dataChannel.send(JSON.stringify(sendData_));
    } catch (e) {
      console.log(e);
      var chunks;
      chunks = chunkSubstr(JSON.stringify(sendData_), 30000);
      chunks.forEach(function(e) {
        appController.dataChannel.send(e);
      });
    }
  }.bind(this));
};
AppController.prototype.newDesigner = function() {
  this.designer = new CanvasDesigner;
  this.designer.widgetHtmlURL = "../widget.html";
  this.designer.widgetJsURL = "js/widget.js";
  if (this.isPc) {
    this.designer.setSelected("marker");
    this.designer.setTools({arrow:true, pencil:true, dragSingle:true, dragMultiple:true, eraser:true, rectangle:true, text:true, image:true, pdf:true, marker:true, lineWidth:true, colorsPicker:true, extraOptions:true, undo:true});
  } else {
    this.designer.setTools({line:false, arrow:false, pencil:false, dragSingle:false, dragMultiple:false, eraser:false, rectangle:false, arc:false, bezier:false, quadratic:false, text:false, image:false, pdf:false, marker:false, zoom:false, lineWidth:false, colorsPicker:false, extraOptions:false, code:false, undo:false});
  }
  this.designer.isLocalMobile = !this.isPc;
  this.designer.isRemoteMobile = this.isRemoteMobile;
  if (this.miniVideo_.classList.value !== "hidden") {
    this.miniVideoClone = this.miniVideo_.cloneNode();
    this.miniVideoClone.srcObject = this.infoBox_.localVideo_.srcObject;
    this.miniVideoClone.id = "mini-video-clone";
    this.miniVideoClone.setAttribute("class", "active");
    this.miniVideoClone.onclick = this.changeMiniVideo_.bind(this);
  }
};
AppController.prototype.onFinishDatachannel = function() {
  if (!this.dataChannel) {
    console.error("not finished datachannel");
    return;
  }
  if (this.dataChannel.readyState !== "open") {
    console.error("channel not opened yet");
    return;
  }
  this.newDesigner();
  this.dataChannel.onmessage = function(event) {
    ...
    try {
      if (typeof JSON.parse(event.data) == "object") {
        ...
        if (parsedData.type === "canvas") {
          this.designer.syncData(parsedData.data);
        } else {
          if (parsedData.type === "chat") {
            this.syncChatMsg(parsedData.data);
          }
        }
      }
    } catch (e) {
      ...
      try {
        var tempStr = this.tempArray.join("");
        if (typeof JSON.parse(tempStr) == "object") {
        ...
        console.log("still not full json");
      }
    }
  }.bind(this);
  if (this.isPc) {
    this.designer.addSyncListener(function(data) {
      ...
      sendData_ = {type:"canvas", data:data};
      try {
        appController.dataChannel.send(JSON.stringify(sendData_));
      } catch (e) {
        console.log(e);
        var chunks;
        chunks = chunkSubstr(JSON.stringify(sendData_), 30000);
  ...
  this.hide_(this.miniVideo_);
  if (this.miniVideoClone) {
    document.body.insertBefore(this.miniVideoClone, this.videosDiv_);
  }
  this.designer.appendTo(document.getElementById("videos"));
  this.activate_(document.getElementById("canvasDesigner"));
};
...
AppController.prototype.syncChatMsg = function(data) {
  if (!this.sharingDiv_ || typeof data !== "string") {
    return;
  }
  if (this.call_.channel_.websocket_.readyState !== 1) {
    return;
  ...
  if (!document.getElementById("chatbox")) {
    var chatbox = document.createElement("ul");
    chatbox.id = "chatbox";
    this.sharingDiv_.appendChild(chatbox);
  }
  var info = document.createElement("li");
  info.innerText = data;
  document.getElementById("chatbox").insertBefore(info, document.getElementById("chatbox").firstElementChild);
  if (this.isPc) {
    this.showChatMsg_();
  } else {
    this.toggleOpacity();
...
AppController.prototype.syncResolution = function(data) {
  if (!this.resolutionDict) {
    return;
  }
  if (this.call_.channel_.websocket_.readyState !== 1) {
    return;
  ...
  if (data.type === "resolution") {
    if (typeof data.data == "object") {
      parsedData_ = data.data;
      try {
        if (parsedData_.hasOwnProperty("localw") && parsedData_.hasOwnProperty("localh")) {
          this.resolutionDict["remotew"] = parsedData_["localw"];
          this.resolutionDict["remoteh"] = parsedData_["localh"];
          var sendData_;
          sendData_ = {type:"onResolution", data:this.resolutionDict};
          this.call_.channel_.send(JSON.stringify(sendData_));
        }
      } catch (e) {
        console.warn(e);
    ...
    window.resolutionDict = this.resolutionDict;
    if (this.designer) {
      this.designer.changeSize();
    }
  } else {
    if (data.type === "onResolution") {
      if (typeof data.data == "object") {
        parsedData_ = data.data;
        try {
          if (parsedData_.hasOwnProperty("localw") && parsedData_.hasOwnProperty("localh")) {
...
AppController.prototype.onRemoteFinishCanvas = function() {
  if (!this.dataChannel) {
    console.error("not finished datachannel");
    return;
  }
  ...
  this.designer.renderStream();
  this.designer.sync();
};
AppController.prototype.onDataChannel_ = function(event) {
  if (!this.call_.pcClient_ || !this.call_.pcClient_.pc_) {
    return;
  }
  ...
  this.dataChannel.onopen = function() {
    this.onFinishDatachannel();
    console.log("finish data build");
    this.call_.channel_.send("finishCanvas");
  }.bind(this);
};
AppController.prototype.onDestoryCanvas = function() {
  if (!this.designer) {
    return;
  }
  this.designer.destroy();
...
AppController.prototype.onCreateCanvas = function() {
  if (!this.call_.pcClient_.pc_) {
    return;
  }
  if (this.call_.channel_.websocket_.readyState !== 1) {
    console.error("not finished websocket");
  ...
  this.call_.channel_.send("createCanvasOk");
};
AppController.prototype.createDataChannel_ = function() {
  if (!this.call_.pcClient_.pc_) {
    return;
  }
  this.dataChannel = this.call_.pcClient_.pc_.createDataChannel("dataChannel");
  this.call_.dataChannel = this.dataChannel;
  this.dataChannel.onopen = function() {
    this.onFinishDatachannel();
    console.log("finish data build");
...
AppController.prototype.onVideoSizeChanged = function() {
  if (this.sizeChangeTimer) {
    window.clearTimeout(this.sizeChangeTimer);
  }
  this.sizeChangeTimer = window.setTimeout(function() {
    this.resolutionDict["localw"] = this.videosDiv_.scrollWidth;
    this.resolutionDict["localh"] = this.videosDiv_.scrollHeight;
    console.log(this.resolutionDict);
    var sendData_;
    sendData_ = {type:"resolution", data:this.resolutionDict};
    this.call_.channel_.send(JSON.stringify(sendData_));
    window.resolutionDict = this.resolutionDict;
...
AppController.prototype.onMobileText = function() {
  if (this.sizeChangeTimer) {
    window.clearTimeout(this.sizeChangeTimer);
  }
  this.sizeChangeTimer = window.setTimeout(function() {
...
AppController.prototype.RemoteMobile = function() {
  this.isRemoteMobile = true;
  trace("isRemoteMobile : " + this.isRemoteMobile);
};
...
var Call = function(params) {
  ...
  if (params.isKurento) {
    this.channel_ = new SignalingChannel(params.kurentoWss, params.wssPostUrl);
  ...
  this.pcClient_ = null;
  this.localStream_ = null;
  if (params.isCutScreen) {
    this.isCutScreen = params.isCutScreen;
  } else {
    this.isCutScreen = false;
  }
  if (params.isKurento) {
    this.isKurento = params.isKurento;
  } else {
    this.isKurento = false;
  }
  ...
  this.onDataChannel_ = null;
  this.syncCanvasData_ = null;
  this.channel_.syncCanvasData = this.syncCanvasData_;
...
Call.prototype.requestMediaAndIceServers_ = function() {
  ...
  if (this.channel_.roomId_ && this.channel_.clientId_) {
    if (this.params_.isCutScreen && !this.params_.mediaConstraints.video && !this.params_.mediaConstraints.audio) {
      this.sendSignalingMessage_({"type":"mediaAdd", "mediaType":"s"});
    } else {
      if (!this.params_.isCutScreen && (this.params_.mediaConstraints.video || this.params_.mediaConstraints.video)) {
        this.sendSignalingMessage_({"type":"mediaAdd", "mediaType":"c"});
        this.getMediaPromise_ = this.maybeGetMedia_();
      } else {
        this.sendSignalingMessage_({"type":"mediaAdd", "mediaType":"cs"});
        this.getMediaPromise_ = this.maybeGetMedia_();
      }
    }
  }
...
Call.prototype.hangup = function(async) {
  ...
  if (this.channel_.registered_) {
    if (ifStarted) {
      ...
      steps.push({step:function() {
        this.channel_.send(JSON.stringify({type:"bye"}));
      }.bind(this), errorString:"Error sending refuse:"});
    }
  } else {
    steps.push({step:function() {
      this.channel_.refuse(JSON.stringify({type:"occupied"}));
    }.bind(this), errorString:"Error sending refuse:"});
  }
  steps.push({step:function() {
...
Call.prototype.connectToRoom_ = function(roomId) {
  ...
  if (!this.channel_.websocket_ || this.channel_.websocket_.readyState !== WebSocket.OPEN) {
...
Call.prototype.maybeGetMedia_ = function() {
  var needStream = this.params_.mediaConstraints.audio !== false || this.params_.mediaConstraints.video !== false || this.params_.isCutScreen !== false;
  ...
  if (needStream) {
    if (!this.params_.isCutScreen && (this.params_.mediaConstraints.video || this.params_.mediaConstraints.audio)) {
      ...
      mediaPromise = navigator.mediaDevices.getUserMedia(mediaConstraints).catch(function(error) {
        ...
        window.appController.infoBox_.localVideoSrc = stream;
      ...
      if (this.params_.isCutScreen && !this.params_.mediaConstraints.video) {
        screenPromise = navigator.mediaDevices.getDisplayMedia().catch(function(error) {
          alert(error);
        }).then(function(stream) {
          this.onCutScreenSuccess_(stream);
        }.bind(this));
...
Call.prototype.onCutScreenSuccess_ = function() {
  appController.miniVideo_.srcObject = this.pcClient_.pc_.getLocalStreams()[0];
};
Call.prototype.getScreenShareTrack = function(shareSrc) {
  var sender = this.pcClient_.pc_.getSenders().filter(function(se) {
    if (shareSrc.getVideoTracks().length === 1) {
      if (se.track.id === shareSrc.getVideoTracks()[0].id) {
        return se;
      }
    } else {
      trace.error("screen share source has more than one video tracks");
      if (se.track.id === shareSrc.getVideoTracks()[0].id) {
        return se;
      }
    }
...
Call.prototype.removeTrack = function(track) {
  if (!this.pcClient_.pc_) {
    trace.error("no peer connection found");
    return;
  }
  if (typeof track !== "object" || track.length !== 1) {
    trace.error("sender array have more than one elements");
    return;
  }
  this.pcClient_.pc_.removeTrack(track[0]);
};
...
Call.prototype.createPcClient_ = function() {
  ...
  this.pcClient_.ontrackremoved = this.ontrackremoved;
  ...
  this.pcClient_.dataChannel = this.dataChannel;
  this.pcClient_.pc_.ondatachannel = this.onDataChannel_;
...
Call.prototype.joinRoom_ = function() {
  return new Promise(function(resolve, reject) {
    ...
    if (this.params_.user) {
      var body = '{"client_id":"' + this.params_.user + '"}';
    }
    sendAsyncUrlRequest("POST", path, body).then(function(response) {
...
Call.prototype.onKurentoRecvSignalingChannelMessage_ = function(msg) {
  this.maybeCreatePcClientAsync_().then(this.pcClient_.receiveSignalingMessage(msg));
};
...
var InfoBox = function(infoDiv, call, versionInfo) {
  ...
  this.localScreenShareSrc = null;
  this.remoteScreenShareSrc = null;
};
...
var PeerConnectionClient = function(params, startTime) {
  this.params_ = params;
  this.params_.remoteVideo = null;
  this.params_.remoteScreenShare = null;
  this.dataChannel = null;
  this.startTime_ = startTime;
  ...
  this.pc_.onremovestream = this.onTrackRemoved_.bind(this);
  ...
  if (messageObj.type === "answer" || messageObj.type === "offer") {
    ...
    if (messageObj.type === "candidate") {
      ...
      if (messageObj.type === "bye") {
        ...
        if (messageObj.type === "mediaAdd") {
          if (messageObj.mediaType === "s") {
            this.params_.remoteScreenShare = true;
          } else {
            if (messageObj.mediaType === "c") {
              this.params_.remoteVideo = true;
            } else {
              if (messageObj.mediaType === "sc") {
                this.params_.remoteVideo = true;
                this.params_.remoteScreenShare = true;
              }
            }
          ...
          if (messageObj.type === "mediaDel") {
            if (messageObj.mediaType === "s") {
              this.params_.remoteScreenShare = false;
            } else {
              if (messageObj.mediaType === "c") {
                this.params_.remoteVideo = false;
              } else {
                if (messageObj.mediaType === "sc") {
                  this.params_.remoteVideo = false;
                  this.params_.remoteScreenShare = false;
...
PeerConnectionClient.prototype.onTrackRemoved_ = function() {
  if (!this.pc_) {
    return;
  }
...
PeerConnectionClient.prototype.onRemoteStreamAdded_ = function(event) {
  if (this.onremotestreamadded) {
    this.onremotestreamadded(event.streams);
  }
};
PeerConnectionClient.prototype.onNegotiationNeeded_ = function() {
  if (!this.pc_ || !this.pc_.remoteDescription || !this.pc_.localDescription) {
    return;
  }
  trace("NEGOTIATIONING!!!");
  trace("Signaling state changed to: " + this.pc_.signalingState);
  if (this.pc_.signalingState !== "stable") {
    trace("cant renegotiate since unstable");
    return;
  }
    ...
    this.websocket_.onmessage = function(event) {
      ...
      if (event.data === "ping") {
        ...
        if (event.data === "occupied") {
          ...
          if (message.msg === "canvas") {
            trace("sync data..");
            this.syncCanvasData(message.data);
            return;
          } else {
            if (message.msg === "createCanvas") {
              window.appController.onCreateCanvas();
              return;
            } else {
              if (message.msg === "destroyCanvas") {
                window.appController.onDestoryCanvas();
                return;
              } else {
                if (message.msg === "createCanvasOk") {
                  window.appController.onFinishDesigner();
                  return;
                } else {
                  if (message.msg === "finishCanvas") {
                    window.appController.onRemoteFinishCanvas();
                    return;
                  }
                }
              }
          ...
          if (typeof JSON.parse(message.msg) == "object") {
            var parsedData_;
            parsedData_ = JSON.parse(message.msg);
            if (parsedData_.type === "chat") {
              window.appController.syncChatMsg("\u5bf9\u65b9\uff1a" + parsedData_.data);
              return;
            } else {
              if (parsedData_.type === "resolution") {
                window.appController.syncResolution(parsedData_);
                return;
              } else {
                if (parsedData_.type === "onResolution") {
                  window.appController.syncResolution(parsedData_);
                  return;
                } else {
                  if (parsedData_.type === "isMobile") {
                    window.appController.RemoteMobile();
                    return;
                  }
                }
              }
...
SignalingChannel.prototype.wait = function(roomId, clientId) {
  if (this.registered_) {
  ...
  var waitMessage = {cmd:"wait", roomid:this.roomId_, clientid:this.clientId_};
  this.websocket_.send(JSON.stringify(waitMessage));
  trace("client waiting");
};
...
SignalingChannel.prototype.refuse = function(message) {
  if (loadingParams.entrance !== "qtalk" && !this.roomId_ || !this.clientId_) {
    trace("ERROR: SignalingChannel has not registered.");
    return;
  }
  trace("C->WSS: roomId" + this.roomId_);
  trace("C->WSS: clientId" + this.clientId_);
  trace("C->WSS: " + message);
  var wssMessage = {cmd:"refuse", msg:message};
...
var KurentoSignalingChannel = function(wssUrl) {
  this.wssUrl_ = wssUrl;
  this.roomId_ = null;
  this.clientId_ = null;
  this.websocket_ = null;
...
KurentoSignalingChannel.prototype.open = function() {
  if (this.websocket_) {
    trace("ERROR: SignalingChannel has already opened.");
    return;
  }
  ...
  return new Promise(function(resolve, reject) {
    ...
    this.websocket_.onmessage = function(event) {
      ...
      if (typeof JSON.parse(event.data) == "object") {
        var parsedData_;
        parsedData_ = JSON.parse(event.data);
        if (parsedData_.jsonrpc !== "2.0" || !parsedData_.result) {
          trace.error("\u9519\u8bef\u7684jsonrpc\u7248\u672c\u6216\u670d\u52a1\u5668\u6ca1\u6709\u8fd4\u56de");
          return;
        }
        if (!!parsedData_.error) {
          trace.error("kurento media server \u9519\u8bef: " + parsedData_.error);
          return;
        }
        if (typeof parsedData_.result == "object") {
          if (parsedData_.result.hasOwnProperty(key)) {
          }
          if (!this.session_id && parsedData_.session) {
            this.session_id = parsedData_.session;
          }
        } else {
          trace.warn("\u5947\u602a\u7684 result \u7c7b\u578b : " + parsedData_.result);
        }
        if (parsedData_.type === "chat") {
          window.appController.syncChatMsg("\u5bf9\u65b9\uff1a" + parsedData_.data);
          return;
...
function goHangupUrl() {
  var tempUrl = window.location.href;
  if (window.location.search) {
    tempUrl += "&status=hangup";
    return tempUrl;
  } else {
    tempUrl += "?status=hangup";
    return tempUrl;
